<template lang="html">
  <div class="ui segment counter basic" id="MultiIntelligenceRoomCounter">
    ...
    <div class="ui segment center aligned basic">
        <button class="ui labeled icon button olive" @click="$emit('userKickStart')" >
          <i  class="call icon"></i>
          进入教室调试
        </button>
    </div>
...
export default {
  ...
  computed: {
    courseHour () {
      ...
      let minutes = bookTimeObj.getMinutes() < 9 ? '0'+bookTimeObj.getMinutes() : bookTimeObj.getMinutes()
      let seconds = bookTimeObj.getSeconds() < 9 ? '0'+bookTimeObj.getSeconds() : bookTimeObj.getSeconds()

      return bookTimeObj.getHours()+':'+ minutes +':'+ seconds
    },
    courseFullTime () {
