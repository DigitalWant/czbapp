<template lang="html">
  <div class="ui container segment payment very padded" v-if="USER_SIGN_IN_INFO!==undefined">
    <table class="ui padded table" id="cartList">
      ...
      <tbody>
        <tr v-for="product in cartList">
          ...
          <td> <el-input-number v-model="product.goods_qty" :disabled="false" @change="quantityChange" :min="1" :max="10" ></el-input-number></td>
    ...
    <div class="ui segment" v-if="shippingInfo">
    ...
    <el-dialog title="请选择商品" v-model="showSelectBox" size="large" >
      <div class="ui cards centered aligned">
        <div class="card "  v-for="(course, index) in courseList">
          <div class="content">
            <div class="header">{{course.name}}</div>
            <div class="image">
              <img :src="course.mobile_img" class="ui image fluid">
            </div>
            <div class="description">
              <div class="ui divider"></div>
              <div class="meta"><strike> 原价:{{course.original_price}} </strike></div>
              <h2 class="ui olive header">现价:{{course.current_price}}</h2>
              <button class="ui button olive" type="button" name="button" @click="selectCourse(course)"> 购买课程</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="ui container" v-if="shipInfo===true">
      <selectable-button
...
export default {
  ...
  data () {
    return {
      ...
      chargeItem: {
        ...
        goods_cat: 'course',
        shipping_fee:0,
        address_id: 'selectedAddressId(无需地址商品)',
        return_url:'www.czbapp.com/payment/multiIntelligence/result', //支付成功后将返回的地址
      },
      product_api: '//mp.yuningwang.com/tczmall/itemList/desktop/0/1/10/1',
      showSelectBox: true,
      selectedProduct:'',
      selectedAddress:'',
      shipInfo: [],
  ...
  computed:{
    ...
    //     goods_id:'121',
    //     goods_qty:1,
    //     user_id: vm.USER_SIGN_IN_INFO.id,
    //     sourceType:'web',
    //     req_channel:'web',
    //     pay_type: 'alipay_pc_direct',
    //     shipping_method:'SMS',
  ...
  methods: {
    ...
    selectCourse(item){
      let vm = this
      vm.showSelectBox = false
      vm.selectedProduct = item.id
      ...
      vm.cartList[0].price=item.current_price
      vm.cartList[0].name=item.name
      // vm.$forceUpdate()
    ...
    checkout(){
      ...
      if (vm.cartList.length==0){
        ...
        vm.chargeItem.user_id = vm.USER_SIGN_IN_INFO.id
        vm.chargeItem.goods_id = vm.selectedProduct
        vm.chargeItem.goods_cat = 'intel'

        vm.$store.dispatch('PAYMENT_CHECKOUT',vm.chargeItem).then(res=>{
    ...
    getCourseInfo() {
      let vm = this
      vm.$http
      .get('//mp.yuningwang.com/tczmall/itemList/desktop/0/1/10/1',{headers: {Accept: "*/*"}})
      .then(res=>{
        // console.log(res.body);
        let resData = JSON.parse(JSON.stringify(res.body))['data']
        vm.courseList[0] = resData[0]
        vm.courseList[1] = resData[1]
        vm.$forceUpdate()
      })
  ...
  created () {
    let vm = this
    vm.$store.state.cart.PAYMENT_STEP = 1
    vm.cartList.user_id = vm.USER_SIGN_IN_INFO != undefined ? vm.USER_SIGN_IN_INFO.user_id : ''
    // Get user history address list

    if (vm.$store.state.userRegLog.USER_SIGN_IN_INFO != undefined) {
      vm.getCourseInfo()

      // vm.$store.dispatch('USER_GET_HISTORY_RECEIVE_ADDRESS',vm.$store.state.userRegLog.USER_SIGN_IN_INFO.id).then(res=>{
      //   vm.$nextTick(function(){
      //     let data =  JSON.parse(JSON.stringify(res.body))
      //
      //     vm.shipInfo = data.map(function(item){
      //       item.default_addr = item.default_addr==1?true:false
      //       return item
